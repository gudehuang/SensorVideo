...
public class MainActivityJavaCameraView extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case CANCEL_PROGRESSDIALOG:
                    ...
                    progressDialog = null;
                    Toast.makeText(MainActivityJavaCameraView.this, filename + "已保存", Toast.LENGTH_LONG).show();
                default:
        ...
        public void onManagerConnected(int status) {
            switch (status) {
                case BaseLoaderCallback.SUCCESS:
                    ...
                    ( (CameraJavaView)cameraview).setResolution(720,1280);
                    break;
                default:
    ...
    public Mat onCameraFrame(final CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        Core.transpose(mat, mat);//transpose（src,targe） 将目标倒转 如840*480变为480*840
        Core.flip(mat, mat, 1);
        if (isStart) {
        ...
        switch (item.getItemId()) {
            ...
            case 300:
              //  showFilesDialog(this, dataDir, "", ShowActivity.class);
                break;
            case  400:
                final List<android.hardware.Camera.Size> sizeList=((CameraJavaView)cameraview).getResolutionList();
                AlertDialog.Builder builderSize=new AlertDialog.Builder(this);
                String[] stringList=new String[sizeList.size()];
                int i=0;
                final int[] selectItem = {0};
                for (android.hardware.Camera.Size size:sizeList)
                {
                    stringList[i++]=size.height+"*"+size.width;
                }
              builderSize.setSingleChoiceItems(stringList, 0, new DialogInterface.OnClickListener() {
                  @Override
                  public void onClick(DialogInterface dialogInterface, int i) {
                  selectItem[0] =i;
                  }
              });
                builderSize.setTitle("设置分辨率");
                builderSize.setPositiveButton("确认", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
                      ((CameraJavaView) cameraview).setResolution(sizeList.get(selectItem[0]));
                    }
                });
                builderSize.show();
        ...
        switch (view.getId()) {
            case R.id.start_btn:
                if (isStart) {
                    ...
                    buttonOnclick.OnclickStart(position, getResources().getString(R.string.btn_name1), Color.RED, VideoReader.TYPE_UNKNOW);
    ...
    class ButtonOnclick {
        ...
        public void OnclickStart(int position, String text, int color, int type) {
            ...
            videoRecoder = new VideoRecoderList(MainActivityJavaCameraView.this, type, filename, VideoWriter.fourcc('M', 'J', 'P', 'G'), 18, framesize);
